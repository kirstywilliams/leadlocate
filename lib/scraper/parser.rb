module Parser
  ...
  class Lead
    ...
    def self.get_profile(url)
      ...
      Rails.logger.info("Unable to get profile for lead at #{url}. \n#{e.message}")
    end
    ...
    def skills
      ...
      @skills ||= @page.search('.skill-pill .endorse-item-name-text').map do |item|
        skill = item.text.strip if item.text
    ...
    def websites
      ...
      @websites ||=  @page.search('#overview-summary-websites').flat_map do |site|
        ...
        { :site => CGI.parse(URI.parse(url).query)['url'][0] }
      end
    ...
    def groups
      ...
      @groups ||= @page.search('.groups-name').map do |item|
        ...
        link = (item.at('a')['href'] =~ /http:\/\/www.linkedin.com/).nil? ? "http://www.linkedin.com#{item.at('a')['href']}" : item.at('a')['href']
        { :name => name, :link => link }
      end
    ...
    def get_company_details(link)
      ...
      if node_2
        node_2.search('p').zip(node_2.search('h4')).each do |value, title|
          if title.text.gsub(' ', '_').downcase == "type"
            result[:company_type] = value.text.strip
          else
